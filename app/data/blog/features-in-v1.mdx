...
The `DEFAULT_LAYOUT` for blog posts page is set to `PostSimple`.
...
I have struggled with adding dynamic `require` in Remix so this will have to do for now.
...
switch (layout) {
  ...
  case "AuthorLayout":
    return (
      <AuthorLayout children={children} authorFrontMatter={authorFrontMatter} />
    );
  case "ExampleLayout":
